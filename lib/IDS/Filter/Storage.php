...
class Storage
{
     ...
     *
     * @param Init|object $init IDS_Init instance
     *
     * @param CacheInterface $magentoCache
     */
    final public function __construct(Init $init, CacheInterface $magentoCache)
    {
        if ($init->config) {
            $type         = $init->config['General']['filter_type'];
            $this->source = $init->getBasePath(). $init->config['General']['filter_path'];

            $this->cache = $magentoCache;
